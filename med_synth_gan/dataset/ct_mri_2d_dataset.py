...
class CtMri2DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mri_vol_idx = np.random.randint(0, len(self.mri_volumes))
        mri_slice_idx = np.random.randint(0, self.mri_volumes[mri_vol_idx].shape[self.slice_axis])
        ...
        # Clip and normalize
        mri_img = np.clip(mri_img, a_min=0, a_max=p99)
        mri_img = (mri_img - 0) / (p99)  # Normalize to [0, 1]
        # Apply MRI transformations with improved normalization
        mri_tensor = torch.Tensor(mri_img).unsqueeze(0).unsqueeze(0)
        mri_tensor = torch.nn.functional.interpolate(mri_tensor, size=(256, 256), mode='bilinear')[0]
